<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Modifier une année</h2>
    </x-slot>
    ...
    <div class="af-hero-bar">
        <div class="container position-relative">
            ...
            <h1 style="font-family:'Playfair Display',serif;font-size:clamp(1.5rem,3vw,2rem);font-weight:900;color:#fff;margin:0;">
                Modifier <span style="color:var(--gold);">{{ $niveau->nom }}</span>
            </h1>
        </div>
    </div>
                        ...
                        <form method="POST" action="{{ route('admin.niveaux.update', $niveau) }}">
                            ...
                            <div class="mb-5">
                                ...
                                <select id="parcours_id" name="parcours_id" class="af-select" required>
                                    ...
                                    <option value="{{ $parcour->id }}" @selected(old('parcours_id', $niveau->parcours_id) == $parcour->id)>
                                        {{ $parcour->nom }}
                                    </option>
                                    @endforeach
                                </select>
